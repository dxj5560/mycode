...
# 矩阵的20条性质验证——格式修改，以及identical()与all.equal()的区别和apply()与taplly()的区别
...
# 20 验证矩阵乘积的行列式，all.equal()与identical()的区别
det(A %*% B) == det(A)*det(B)
# 这里显示不相等，但是用all.equal()能判断出二者相等
all.equal(det(A %*% B), det(A)*det(B))
# 而当用identical()判断时,二者结果并不相等
identical(det(A %*% B), det(A)*det(B))
# identical()用来比较二者精确相等，而all.equal()是近似相等
...
# 可以知道apply()与tapply()都可以用来批量处理数据，前者用位置来划分，后者可以自定义标签来划分。